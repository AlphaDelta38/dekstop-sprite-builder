  ...
  return (
    <Box className={styles.container}>
      ...
      {currentFolder.folders.map((folder)=>
        <FolderComponent key={folder.name + folder.files.length + folder.folders.length} folder={folder} openFolderCallback={() => openFolderCallback(folder)} />
      )}

      {renderFile && currentFolder.files.map((file, index)=>
        <FileComponent key={file.name + file.size + file.type + index} file={file} />
      )}
